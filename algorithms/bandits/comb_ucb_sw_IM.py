...
class ucbSW(ucbIM):
    ...
    """
    DONT KNOW IF THIS IS CORRECT, LAST RESORT
    def estimate_probabilities_optimized(self, episode: npt.NDArray):
        ...
        for node_index in range(self.n_nodes):
            ...
            idx_u_active = np.argwhere(episode[:, node_index] == 1).reshape(-1)  # t_u_k
            if idx_u_active.size > 0 and idx_u_active > 0:
                active_nodes_in_prev_step = episode[idx_u_active - 1, :].reshape(-1)
                for w in range(self.n_nodes):
                    self.credits_collected[node_index][w] = np.append(self.credits_collected[node_index][w], active_nodes_in_prev_step[w] / np.sum(active_nodes_in_prev_step))
            # for every node different from node_index,
            for v in range(0, self.n_nodes):
                if v != node_index:
                    idx_v_active = np.argwhere(episode[:, v] == 1).reshape(-1)
                    if idx_v_active.size > 0 and (idx_u_active.size == 0 or idx_v_active < idx_u_active):
                        self.occur_v_active[node_index][v] = np.append(self.occur_v_active[node_index][v], 1)
                    else:
                        self.occur_v_active[node_index][v] = np.append(self.occur_v_active[node_index][v], 0)
                else:
                    self.occur_v_active[node_index][v] = np.append(self.occur_v_active[node_index][v], 0)
            estimated_prob = np.array(self.credits_collected[node_index][:][-self.window_size:]).sum(axis=1) / \
                             np.array(self.occur_v_active[node_index][:][-self.window_size:]).sum(axis=1)
